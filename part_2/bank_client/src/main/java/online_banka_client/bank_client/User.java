...
class User
{
	...
	static User registration() {
		...
		UserInfo userInfo = new UserInfo(username, password, choice);
		try {
			App.rest.postForObject(App.serverAddr + "/users/registration", userInfo, boolean.class);
		} catch (RestClientException e) {
	...
	static User login() {
		...
		while (true) {
			...
			UserInfo userInfo = new UserInfo(username, passwordAttempt, '-');
			try {
				response = App.rest.postForObject(App.serverAddr + "/users/login", userInfo, boolean.class);
				// prijava uspjesna
